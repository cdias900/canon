...
| Platforms | **iOS, Android and desktop.** iOS and macOS have been played since the start; **Android was first built and played on 03/09/2026** — an IL2CPP/ARM64 APK on an arm64 emulator (`tools/android-emu.sh`), through the opening to character creation and the village. The Android module for this Unity was installed that day; before it, no APK had ever existed. **iOS has run on a physical iPhone 16 Pro since 04/09/2026** (`tools/ios-device.sh`, signed with a personal team; the phone opened it, the harness has never driven it; **the first play on it found the wall segments built at the south edge of the city, off the straight run** — fixed 04/09/2026, `wall_row` in `map.json`, criterion 20 — and a device build ships without the Development Console since that day); **no physical Android device yet**, and that half of the definition of done stays open. |
