...
namespace SheepGate.Player
{
    ...
    /// and the calendar. A wall stage is reached by working any segment, by the night crew, and by
    /// the day-three sequence closing the gate, so it is nobody's proxy for a single scored action;
    /// the player watches that number rise on the wall itself whether or not a wardrobe exists.
    ///
    /// ==================================================================================
    ...
    ///   * <see cref="GameState.day"/> only advances;
    ...
    public static class UnlockEvaluator
    {
        ...
        /// <see cref="ForbiddenFlags"/> and against a fresh grep of <c>AwardOnce</c>,
        /// <c>AddVocation</c> and <c>"grants"</c> in dialogue.json — the scoring sites move.
        /// </summary>
        ...
        static readonly Dictionary<string, string> ForbiddenFlags = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            { "watch_posted_d2", "posting the watch on day 2 grants Prophet points" },
            { "watch_posted_d1", "the same verb that scores on day 2" },
        ...
        /// as the lowest across every segment is barely reachable in a three-day run, and an item
        /// nobody can ever unlock is a padlock that reads as a tease. The highest is what the player
        /// has actually managed somewhere on the wall, and they can see it standing there.
        /// </summary>
        ...
        static void LogRefusedDiscovery(string id)
        {
            ...
            string reason;
            if (ForbiddenFlags.TryGetValue(trimmed, out reason))
            {
