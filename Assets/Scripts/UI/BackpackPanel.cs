...
namespace SheepGate.UI
{
    ...
    public sealed class BackpackPanel : MonoBehaviour
    {
        ...
        // ------------------------------------------------------------------ locale keys
        // Every word on this screen is a key, and after the row extraction this file owns only the
        // chrome: the title and the way out. Both are ui.json, read with Loc.T.
        //
        ...
        /// <summary>
        /// The lane the permanent scrollbar occupies beside a wardrobe list, and it is aliased off
        ...
        /// <summary>
        /// Width of the figure's box on the stage, derived from its height and the character ratio.
        ///
        /// The ratio comes from <see cref="CharacterFigure.FigureAspect"/> and is not recomputed
        /// here. Both the big figure and every thumbnail are <i>fitted</i> to that ratio rather than
        /// given a size, which is what keeps one number right on a 1080-unit reference and on the
        /// ~977 units a phone actually reports.
        /// </summary>
        static readonly float FigureBoxWidth = FigureBoxHeight * CharacterFigure.FigureAspect;
        ...
        // The five art layers, their draw order, their GameObject names, their sprite lookups and
        // their missing-art fallback bands all live in CharacterFigure now. This sheet used to
        // carry a private copy of every one of them, byte-identical to the copy character creation
        // carried — two screens drawing the same character, with nothing that would ever notice one
        // of the two drifting.
        ...
        /// <summary>
        /// The stage figure's five layers, in draw order, built by <see cref="CharacterFigure"/>.
        /// Null until <see cref="BuildStage"/> runs; every painter this file calls tolerates that.
        /// </summary>
        Image[] _layers;
        ...
        void BuildStage(RectTransform sheetRect)
        {
            ...
            // the head. Sibling order IS the draw order, which is why the stack is built by
            // CharacterFigure rather than parented here — five layers in the right order is the
            // kind of thing that stays right in one file and drifts in two.
            _layers = CharacterFigure.Build(figure);
        ...
        /// NOVO badge, its own thumbnail, its own accessible-name composition and its own text
        /// pinning — all of it a second copy of what character creation lists on the screen the
        /// player sees first. Almost everything that makes the row correct is arithmetic that looks
        /// like nothing when it drifts: the S4 name-line gap that stops "Túnica de carregador"
        /// breaking mid-word, and the rule that every width descends from
        ...
        ///
        /// So the row is built by <see cref="WardrobeRow.Build"/>, from
        /// <see cref="RowMetrics"/>, and what is left here is the three things that genuinely
        /// belong to the backpack rather than to a row:
        /// <list type="number">
        ...
        /// price belongs on. Character creation passes neither.</item>
        ...
        /// <summary>
        /// Repaints the stage figure from the current look.
        ///
        /// One call, because the five layers, their sprite lookups, their shades and their
        /// missing-art fallback bands are <see cref="CharacterFigure"/>'s to decide. The stage and
        /// the row thumbnails below it now degrade identically when an art layer is missing, which
        /// is the one thing a second private copy of this method could not promise.
        /// </summary>
        void RefreshFigure()
        {
            CharacterFigure.Apply(_layers, Look(), FacingDirection.Down);
        }
