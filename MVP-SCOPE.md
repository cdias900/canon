...
| `WorldMapView` | The progression map behind **Mapa**: nine stops, complete / current / locked off `day`, each stage's piece off the catalogue, and a **diary** per stop read off what the day wrote — courses the player laid (`laid_d<n>`), the night's watch and work (`night_work_d<n>`), the choice made (the branch flags), how a contest ended (`contest_outcome_<id>`), and on the last stop whether the gate is still waiting. One thing IS authored per stage: the **objective** (`world.progress_map.objective.<day>`), what the day asks in the fiction, shown for every stop reached or not — never a reading, a prayer or a reference. The diary stays reached-only; a stop not reached shows its objective and says nothing else. |
